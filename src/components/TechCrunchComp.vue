<template>
    ...
    <div v-else-if="filtered" class="content-container">
        <h1 class="mb-2">All the latest Tech Crunch news at your fingertips!</h1>
...
export default{
  ...
  components: {
    ...
    setup(){
        ...
        store.dispatch("fetchTechCrunch");
        const techCrunch = computed(()=> store.state.techCrunch);
        return {
            techCrunch
        }
    },
    computed: {
        filtered: function(){
        if(this.techCrunch == null){
            return this.techCrunch
        } else {
